...
namespace Mdm.Oss.Components { 
    public static class MickString1
    {
        ...
        private const int iUnknown = 99999;
        /* 
        private String PassedMickString = sEmpty;

        MickString1() : base()
        {
            PassedMickString = sEmpty;
        }
        ...
        */
        ...
        public static FieldActionIs FieldActionLast;
        public static FieldModeIs FieldModeLast;
        ...
        public static String FieldLast(this String PassedMickString, String PassedFieldChar) {
            ...
            int iPassedFieldOccurence = -iUnknown;
            sResult = ActionTheField(sResult, PassedFieldChar, iPassedFieldOccurence, FieldActionIs.Get, FieldModeIs.Extract, null);
            return sResult;
        }
        ...
        public static int Count(this String PassedMickString, String PassedFieldChar) {
            String sResult = ActionTheField(PassedMickString, PassedFieldChar, -iUnknown, FieldActionIs.Count, FieldModeIs.None, null);
        ...
        public static String[] SplitLast(this String PassedMickString, String PassedFieldChar) {
            ...
            sMiddle = SplitInTwo(PassedMickString, PassedFieldChar, -iUnknown);
            return sMiddle;
        }
        ...
        public static String[] SplitInTwo(this String PassedMickString, String PassedFieldChar, int iPassedFieldOccurence) {
            String sResult = PassedMickString;
            String[] sMiddle = new String[2];
            sResult = ActionTheField(PassedMickString, PassedFieldChar, iPassedFieldOccurence, FieldActionIs.Get, FieldModeIs.Split, sMiddle);
            return sMiddle;
        }
        #endregion
        ...
        public static void FieldReset() {
            FieldActionLast = FieldActionIs.None;
            sFieldResultLast = sEmpty;
        ...
        public static String ActionTheField(
            ...
            FieldActionIs iPassedFieldAction,
            FieldModeIs iPassedFieldMode, 
        ...
        // {
            String sFieldResult = sEmpty;
            if (iPassedFieldMode == FieldModeIs.Split) {
                sPassedFieldArr[0] = sEmpty;
                sPassedFieldArr[1] = sEmpty;
            }
            // int FieldEndCharacterIndex = 0;
            // int FieldAttributeIndex = 0;
            FieldEndCharacterIndex = iUnknown;
            FieldAttributeIndex = 0;
            FieldLength = 0;
            FieldLocStart = 0;
            FieldLocEnd = 0;
            ...
            for (FieldAttributeIndex = 1; 
                ...
                || iPassedFieldOccurence == -iUnknown) 
                && FieldEndCharacterIndex >= 0); 
                FieldAttributeIndex++
                ) {
                ...
                if (FieldEndCharacterIndex == -1) {
                    ...
                    switch(iPassedFieldAction){
                        case (FieldActionIs.Count):
                            FieldAttributeIndex -= 1;
                            return sEmpty;
                        case(FieldActionIs.Get):
                            FieldLocEnd = PassedMickString.Length;
                            if (FieldAttributeIndex == iPassedFieldOccurence || iPassedFieldOccurence == -iUnknown) {
                                if (iPassedFieldOccurence > 1 || iPassedFieldOccurence == -iUnknown) {
                                    ...
                                    if (iPassedFieldMode == FieldModeIs.Split) {
                                        if (iPassedFieldOccurence == -iUnknown) {
                                            if (FieldLocStart > 0) {
                            ...
                            break;
                        case (FieldActionIs.Add):
                            sFieldResult = PassedMickString;
                            break;
                        case (FieldActionIs.Delete):
                            sFieldResult = PassedMickString;
                            break;
                        default:
                            sFieldResult = PassedMickString;
                    ...
                    break;
                } else if (FieldEndCharacterIndex >= 0 && FieldAttributeIndex < iPassedFieldOccurence || iPassedFieldOccurence == -iUnknown) {
                    switch(iPassedFieldAction){
                        case (FieldActionIs.Count):
                        case (FieldActionIs.Get):
		...
		/// </remarks> 
        public static String ActionTheField(this String PassedMickString, 
            int iPassedFieldLevel, int iPassedAttibute, int iPassedMultivalue, int iPasseedSubvalue, int iListPassedValue, int iListPassedSubvalue, int iListPassedCsv,
            FieldActionIs iPassedFieldAction, FieldModeIs iPassedFieldMode, string[] sPassedFieldArr)
        {
            if (DelType == null) { 
                SetDelDefault();
                FieldReset();
            ...
            FieldLvm = iListPassedValue; // = "*";
            FieldLsvm = iListPassedSubvalue; // = "@";
            FieldLcsvm = iListPassedCsv; // = ",";
            ...
            //
            String sResult = PassedMickString;
            FieldModeIs iTempFieldMode;
            int iFieldLevel;
            //
            if (iPassedAttibute > 0) {
                iFieldLevel = 1; 
                if (iPassedFieldLevel == iFieldLevel) { iTempFieldMode = iPassedFieldMode; } else { iTempFieldMode = FieldModeIs.Extract; }
            ...
            if (iListPassedValue > 0) {
                iFieldLevel = 4;
                if (iPassedFieldLevel == iFieldLevel) { iTempFieldMode = iPassedFieldMode; } else { iTempFieldMode = FieldModeIs.Extract; }
                sResult = ActionTheField(sResult, Lvm, iListPassedValue, iPassedFieldAction, iPassedFieldMode, sPassedFieldArr);
            }
            if (iListPassedSubvalue > 0) {
                iFieldLevel = 5;
                if (iPassedFieldLevel == iFieldLevel) { iTempFieldMode = iPassedFieldMode; } else { iTempFieldMode = FieldModeIs.Extract; }
                sResult = ActionTheField(sResult, Lsvm, iListPassedSubvalue, iPassedFieldAction, iPassedFieldMode, sPassedFieldArr);
            }
            if (iListPassedCsv > 0) {
                iFieldLevel = 6;
                if (iPassedFieldLevel == iFieldLevel) { iTempFieldMode = iPassedFieldMode; } else { iTempFieldMode = FieldModeIs.Extract; }
                sResult = ActionTheField(sResult, Lcsvm, iListPassedSubvalue, iPassedFieldAction, iPassedFieldMode, sPassedFieldArr);
            }
            return sResult;
        }
        ...
        /// </summary>
        /// <param name="PassedMickString">Extended string to examine</param> 
        /// <param name=sEmpty>1 Attr, 2 Value, 3 Sub-value, 4 List Value, 5 List Sub-Value, 6 List Character Value </param> 
        /// <param name=sEmpty>(Get, Delete, Drop, Insert, Add.)</param> 
        /// <param name=sEmpty></param> 
		/// <returns>
		/// </returns>
        /// <remarks>
		/// </remarks> 
        public static String GetField(this String PassedMickString, int iPassedAttibute, int iPassedMultivalue, int iPasseedSubvalue, int iListPassedValue, int iListPassedSubvalue, int iListPassedCsv, int GET) {
            return ActionTheField(PassedMickString, 5, iPassedAttibute, iPassedMultivalue, iPasseedSubvalue, iListPassedValue, iListPassedSubvalue, iListPassedCsv, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        //
        ...
        /// <remarks>
		/// </remarks> 
        public static String GetField(this String PassedMickString, int iPassedAttibute, int iPassedMultivalue, int iPasseedSubvalue, int iListPassedValue, int iListPassedSubvalue, int GET) {
            return ActionTheField(PassedMickString, 5, iPassedAttibute, iPassedMultivalue, iPasseedSubvalue, iListPassedValue, iListPassedSubvalue, 0, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        //
        // Overloads ListValue
        //
        ...
        /// <remarks>
		/// </remarks> 
        public static String GetField(this String PassedMickString, int iPassedAttibute, int iPassedMultivalue, int iPasseedSubvalue, int iListPassedValue, int GET) {
            return ActionTheField(PassedMickString, 4, iPassedAttibute, iPassedMultivalue, iPasseedSubvalue, iListPassedValue, 0, 0, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        ...
        public static String GetField(this String PassedMickString, int iPassedAttibute, int iPassedMultivalue, int iPasseedSubvalue, int GET) {
            return ActionTheField(PassedMickString, 3, iPassedAttibute, iPassedMultivalue, iPasseedSubvalue, 0, 0, 0, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        ...
        public static String GetField(this String PassedMickString, int iPassedAttibute, int iPassedMultivalue, int GET) {
            return ActionTheField(PassedMickString, 2, iPassedAttibute, iPassedMultivalue, 0, 0, 0, 0, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        ...
        public static String GetField(this String PassedMickString, int iPassedAttibute) {
            return ActionTheField(PassedMickString, 1, iPassedAttibute, 0, 0, 0, 0, 0, FieldActionIs.Get, FieldModeIs.Extract, null);
        }
        ...
        public static String DeleteField(this String PassedMickString, int iPassedAttibute) {
            return ActionTheField(PassedMickString, 1, iPassedAttibute, 0, 0, 0, 0, 0, FieldActionIs.Delete, FieldModeIs.Extract, null);
        }
        ...
        public static String InsertField(this String PassedMickString, int iPassedAttibute) {
            return ActionTheField(PassedMickString, 1, iPassedAttibute, 0, 0, 0, 0, 0, FieldActionIs.Insert, FieldModeIs.Extract, null);
        }
        #endregion
    }
}
